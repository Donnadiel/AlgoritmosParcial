...
public class CircularDoublyLinkedList implements List{
    ...
    public boolean containsTwo(Object element) throws ListException {
        ...
        Node aux = first;
        if (size()==1){
            if(util.Utility.compareTwo(aux.data, element)==0){
                return true;
            }else
                return false;
        }
    ...
    public void addInSortedList(Object element) {
        ...
        if (isEmpty()) {
            ...
            if (util.Utility.compare(first.data, newNode.data) > 0) {
                ...
                while (auxiliar.next != first && util.Utility.compare(auxiliar.next.data, newNode.data) < 0) {
    ...
    public void remove(Object element) throws ListException {
        ...
        if(util.Utility.compareTwo(first.data, element)==0){
            ...
            Node aux = first.next;
            while(aux!=last && !(util.Utility.compareTwo(aux.data, element)==0)){
                prev = aux;
                aux = aux.next;
            }
            ...
            if (aux == last && util.Utility.compareTwo(aux.data, element)==0){
    ...
    public void sort2() throws ListException {
        ...
        for (int i = 1; i <= size() ; i++) {
            for (int j = i+1; j <= size() ; j++) {
                if(util.Utility.compareTwo(getNode(j).data, getNode(i).data)<0){
                    Object aux = getNode(i).data;
                    getNode(i).data = getNode(j).data;
                    getNode(j).data = aux;
                }
